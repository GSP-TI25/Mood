...
/**
 * Componente TeamForm.
 * Formulario para crear o editar el perfil de los miembros del equipo.
 *
 * @param {Object} props
 ...
 * @param {Object} [props.memberToEdit] - Datos del miembro si se está en modo edición.
 */
const TeamForm = ({ onSubmitSuccess, onCancel, memberToEdit }) => {
	...
	/**
	 * Maneja la subida de la imagen validando su peso y tipo (MIME type).
	 */
	...
	/**
	 * Procesa el envío de datos al backend asegurando el token JWT y la sanitización.
	 */
	const handleFinalSubmit = async (e) => {
		...
		// Sanitizamos la carga útil antes de enviarla
		...
		try {
			...
			if (response.ok) {
				...
				alert(
					`Error al guardar el miembro: ${errorData.message || "Desconocido"}`,
				);
			}
		} catch (error) {
			console.error(error);
