...
const TeamForm = ({ onSubmitSuccess, onCancel, memberToEdit }) => {
  ...
  const handleFinalSubmit = async (e) => {
    ...
    dataToSend.append('role_key', formData.role_key.trim().toLowerCase()); // Lo forzamos a minúsculas sin espacios
    dataToSend.append('linkedin', formData.linkedin);

    // Solo enviamos imagen si hay una nueva
    if (imageFile) {
      dataToSend.append('image_url', imageFile);
    } else if (!imagePreview && memberToEdit?.image_url) {
      // Si borró la imagen que ya existía (pasar de líder a general)
      // Podrías necesitar lógica extra en el backend, pero por ahora lo dejamos vacío.
      // En una v2, el backend debería entender que si no llega 'image_url' pero antes tenía, debe anularla.
    }
        ...
        <div className='cms-team-form__scroll-area'>
          ...
          <div className='cms-team-form__section'>
            ...
            <div className='cms-team-form__group-row'>
              <div className='cms-team-form__group'>
                <label>ID del Rol (Para traducción)</label>
                <input
                  type='text'
                  name='role_key'
                  ...
                  onChange={handleInputChange}
                  required
                  placeholder='Ej. matthias (en minúsculas)'
                />
                <span style={{ fontSize: '0.75rem', color: '#64748b' }}>
                  Debe coincidir con la clave en tu archivo es.json (ej:
                  "vasco", "disenador_jr")
                </span>
              </div>
              <div className='cms-team-form__group'>
